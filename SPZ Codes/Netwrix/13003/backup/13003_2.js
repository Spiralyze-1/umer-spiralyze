...
const SPZ13003_W_MKTO1310_LOAD = '__spz13003_mkto1310_load_once';

/** MktoForms2.loadForm appends into the same #mktoForm_1310; a second load stacks another full block (extra <style>, Email row, default button). */
function spz13003StripDuplicateMkto1310Markup(formEl) {
	if (!formEl || formEl.id !== 'mktoForm_1310') return;
	const styles = formEl.querySelectorAll('style');
	if (styles.length > 1) {
		let n = styles[1];
		while (n) {
			const next = n.nextSibling;
			formEl.removeChild(n);
			n = next;
		}
		return;
	}
	const emailRows = formEl.querySelectorAll('.mktoFormRow[data-wrapper="Email"]');
	if (emailRows.length < 2) return;
	let n = emailRows[1];
	while (n) {
		const next = n.nextSibling;
		formEl.removeChild(n);
...
/** Thank-you + stored key: always set page key classes. Full inject only if tour block is missing; otherwise re-apply body variant classes after SPA/back (inject skips when .spz_tour_on_Page exists). */
function spz13003EnsureThankYouVariant() {
	const currentPageData = getPageData();
	if (!currentPageData || !currentPageData.url.includes('demo-thank-you')) return;
	const storedPageKey = localStorage.getItem(SPZ_PAGE_VALUE_KEY);
	if (!storedPageKey) return;
	document.body.classList.add(`spz_${storedPageKey}`);
	if (document.body.classList.contains(GLOBAL_BODY_CLASS)) return;
	if (!document.querySelector('.spz_tour_on_Page')) {
...
function injectVariant6012(currentPageData) {
	...
	if (document.querySelector('.c--hero-b')) {
		window[SPZ13003_W_MKTO1310_LOAD] = false;
	...
	function updateCardsTitle(){
		if(document.querySelector('.spz_tour_on_Page + section .f--container .f--row')){
			...
			document.querySelector('.spz_tour_on_Page + section .f--container .f--row .f--col-4:nth-child(2) .c--card-s .c--card-s__subtitle').innerHTML = `Explore real results from real <br> customers using <br> Netwrix.`
	...
	var formLoadInterval = setInterval(() => {
		...
		const mktoHost = document.getElementById('mktoForm_1310');
		if (!mktoHost || !mktoHost.closest('.spz_tour_on_Page')) {
			clearInterval(formLoadInterval);
			return;
		}

		spz13003StripDuplicateMkto1310Markup(mktoHost);

		const alreadyBuilt =
			mktoHost.querySelector('.mktoFormRow') ||
			mktoHost.querySelector('input[name="formid"][value="1310"]') ||
			mktoHost.classList.contains('spz_form_addition_completed');
		if (alreadyBuilt || window[SPZ13003_W_MKTO1310_LOAD]) {
			clearInterval(formLoadInterval);
			return;
		}

		window[SPZ13003_W_MKTO1310_LOAD] = true;
		...
		MktoForms2.loadForm("//lp.netwrix.com", "130-MAN-089", 1310, async (form) => {
				...
				spz13003StripDuplicateMkto1310Markup(formEl);
	...
	if(document.querySelector('.spz_tour_on_Page')){
		document.querySelector('.spz_tour_on_Page').insertAdjacentHTML('afterend', `<div class="spz_heroOuter"></div>`);
		document.querySelector('.spz_heroOuter').insertAdjacentElement('afterbegin', document.querySelector('.spz_tour_on_Page'));
	}
...
waitForElement('body').then(element => {
	...
	window.addEventListener(
		...
		function () {
			spz13003EnsureThankYouVariant();
			const f = document.getElementById('mktoForm_1310');
			if (f && f.closest('.spz_tour_on_Page')) {
				spz13003StripDuplicateMkto1310Markup(f);
				const btn = f.querySelector('.mktoButtonRow .mktoButton');
				if (btn && !btn.classList.contains('spz13003_v')) {
					btn.textContent = 'Instant Access';
					btn.classList.add('spz13003_v');
				}
			}
		},
		false
