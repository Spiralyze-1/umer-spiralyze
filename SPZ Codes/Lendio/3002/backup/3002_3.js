(function () {
  const bodyInterval3001 = setInterval(function () {
    if (document.body &&
      ...
      const mainPath = (window.location.pathname || '').replace(/\/$/, '');
      const allowedPaths = [
        '/sba-loans',
        '/partners-sf',
        '/simple-a',
        '/lp/direct/search-small-business-loans'
      ];
      const isPathAllowed = allowedPaths.some(base => mainPath === base || mainPath === '/en' + base);
      ...
      const isSbaLoansPage = path.includes('/sba-loans') && !path.includes('/sba-loans-pmax');
      if (isSbaLoansPage || path.includes('/partners-sf') || path.includes('/small-business-financing') || path.includes('/partners-lf')) {
