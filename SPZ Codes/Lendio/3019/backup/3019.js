(function () {
  ...
  function cacheDefaultHeroImages(col) {
    ...
    defaultHeroImages = {
      ...
      tablet:
        existingPicture?.querySelector('source[media*="1023"]')?.getAttribute(
          "srcset",
        ) ||
  ...
  function setupHeroPicture() {
    ...
    col.innerHTML = buildHeroPictureMarkup(defaultHeroImages);
    return true;
  }
  ...
  function updateHeroPicture(tileNum) {
    ...
    const images = tileNum ? TILE_HERO_IMAGES[tileNum] : defaultHeroImages;
    if (!images) return;

    const mobileSource = picture.querySelector('source[media*="767"]');
    const tabletSource = picture.querySelector('source[media*="1023"]');
