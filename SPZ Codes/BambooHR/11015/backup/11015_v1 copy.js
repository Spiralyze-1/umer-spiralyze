	...
	const rightColImageInterval11012 = setInterval(function () {
		...
		if (currentConfig.imageReplacements && currentConfig.imageReplacements.length > 0) {
			for (const replacement of currentConfig.imageReplacements) {
				const imgCol = document.querySelector(`${baseSelector}:has(${replacement.id}) .column7.img-col`);
				if (imgCol && !imgCol.hasAttribute('data-spz-replaced')) {
					const resolvedSrc = getDeviceSrc(replacement.src); // ƒ¢€∂ Â pick correct src
		...
		// Skip the default (Natalie Snow) image when an industry was already chosen —
		// otherwise a refresh overwrites the industry-specific quote photo.
		if (currentConfig.quoteImage && !hasStoredIndustryQuote()) {
...
function hasStoredIndustryQuote() {
	var storedState = getStoredPopupState();
	return !!(storedState && storedState.industry && storedState.industry !== 'other');
}

function setQuotePictureHTML(html) {
	var quotePicture = document.querySelector('#base .section.quote-container .quote-wrapper picture');
	if (!quotePicture) return;
	quotePicture.innerHTML = html;
	quotePicture.setAttribute('data-spz-replaced', 'true');
}

function isIndustryContentReady() {
	return !!(
		document.querySelector('.hero_navData') &&
		document.querySelector('main .form.has-content.grid-7-5 .content-col') &&
		document.querySelector('#base .section.quote-container .quote-wrapper picture') &&
		document.querySelector('#base .section.quote-container .quote-wrapper .content') &&
		document.querySelector('.form-col-container > p > strong') &&
...
function applyIndustryFromPopup(industry) {
	...
	if (!navClass) return true;
	if (!isIndustryContentReady()) return false;
	...
	if (storedState) {
		if (storedState.industry) {
			...
			var restoreInterval = setInterval(function () {
				attempts++;
				if (applyIndustryFromPopup(storedState.industry) || attempts > 50) {
...
function applyPopupInert(isInert) {
	var popup = document.querySelector('.spz_11015_popupOuter');
	var consentBanner = document.querySelector('#dg-consent-banner');
	var privacyBanner = document.querySelector('.dg-consent-banner');
	var excluded = [popup, consentBanner, privacyBanner];
...
function clickEvent() {
	document.addEventListener('click', function (event) {
		...
		if (target.classList.contains('nav_item') && !target.classList.contains('all')) {
			...
			var demoLabel = document.querySelector('.form-col-container > p > strong');
			if (demoLabel) demoLabel.textContent = `Get a Demo`;
			var quoteMoveTarget = document.querySelector('.quote-container .logos-wrapper + .spacer-wrapper + .columns-wrapper + .spacer-wrapper + .columns-wrapper + .spacer-wrapper + .columns-wrapper');
			var quoteWrapper = document.querySelector('.quote-wrapper');
			if (quoteMoveTarget && quoteWrapper) {
				quoteMoveTarget.insertAdjacentElement('afterend', quoteWrapper);
			}
		}
