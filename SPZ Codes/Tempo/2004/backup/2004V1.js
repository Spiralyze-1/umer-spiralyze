    ...
    if (document.querySelector("body")) {
      ...
      if (!document.querySelector('body.observed2004')) {
        const observer = new MutationObserver((mutationsList) => {
          // basic modification
          if ((!document.querySelector('.spz_2004_v1') && window.location.href.startsWith("https://www.tempo.io/"))
          ) {
            document.querySelector('body').classList.add("spz_2004_v1")
          }
          // page modification
          if (document.querySelector('.spz_2004_v1') && !document.querySelector('.spz2004_v1')) {
            if(document.querySelector('a[href="/get-started"]')){
              document.querySelectorAll('a[href="/get-started"]').forEach(function (element) {
                element.innerHTML = `Take an Online Tour <span class="shrink-0 size-6 overflow-hidden"><span class="bg-[url('/images/icons/arrow-white.svg')] custom-arrow flex h-6 w-12 -translate-x-6 bg-contain bg-left bg-repeat-x transition-all duration-300 group-hover:translate-x-0 group-disabled:transform-none group-hover/undefined:translate-x-0"></span></span>`;
                element.classList.add("spz2004_v1");
              })
              localStorage.setItem("spz-form-title", "Take an <em> Online Tour</em>");
            }
          }
