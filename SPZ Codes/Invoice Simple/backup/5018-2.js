...
(function () {
  ...
  const config = {
    ...
    assets: {
      bg1440: 'https://res.cloudinary.com/spiralyze/image/upload/f_auto/invoicesimple/5018/bg.webp',
  ...
  function hasQualifyingQuestions() {
    return !!(document.querySelector('.signup .edition_copy') || document.querySelector('.signup .qlf_wrapper'));
  }

  function loadVariant() {
    if (location.pathname !== config.urlMatch) { return; }
    if (!document.querySelector('form') || document.querySelectorAll('form input').length === 0) { return; }

    if (!document.body.classList.contains(config.bodyClass)) {
      applyTracking();
      updateForm();
    }
    ...
    if (!document.querySelector('.signup ' + config.guard)) {
      addSplitPanel();
    }
  }

  function waitForFormAndLoad() {
    if (location.pathname !== config.urlMatch) { return; }
    if (document.querySelector('form') && document.querySelectorAll('form input').length > 0) {
      loadVariant();
      return;
    }
  ...
  function updateForm() {
    ...
    /* On iOS / SPA body class can be stripped — re-add and re-run via observer (5018_tc pattern) */
    const checkBodyClass = new MutationObserver(function () {
      ...
      } else if (location.pathname === config.urlMatch && document.querySelector('form') && !hasQualifyingQuestions()) {
        animateLabels(config.inputs);
        addQualifyingQuestions();
        addSplitPanel();
      }
  ...
  function addQualifyingQuestions() {
    if (hasQualifyingQuestions()) { return; }
    ...
    return (
      '<div class="spz-5018-section">' +
        '<picture class="spz-5018-bg" aria-hidden="true">' +
          ...
          '<img src="' + a.bg1440 + '" alt="Right form BG">' +
        '</picture>' +
  ...
  window.addEventListener('locationchange', function () {
    if (location.pathname === config.urlMatch) {
      waitForFormAndLoad();
    } else if (document.body && document.body.classList.contains(config.bodyClass)) {
      document.body.classList.remove(config.bodyClass, 'spz-hide-qualifying-q');
    }
  });
})();