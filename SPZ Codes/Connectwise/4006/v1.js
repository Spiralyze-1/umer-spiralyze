...
if (document.body) {
  document.body.classList.add('spz_4006_init');
}
...
function spz4006() {
  if (!document.querySelector('body').classList.contains('spz_4006_v')) {
    document.querySelector('body').classList.add('spz_4006_v');
    // DEVELOPER - STEP 2 of 3 - Edit all page content and assets here
    var template_content = {

      // ── LEFT SIDE ──────────────────────────────────────────────
      eyebrow: 'REMOTE MONITORING AND MANAGEMENT (RMM) SOFTWARE',
      headline: 'Manage endpoints remotely.',
      headlineHighlight: 'Streamline IT',

      bullets: [
        { bold: 'Endpoints and networks:', text: ' Automate patching. Access devices and troubleshoot remotely. Get prioritized alerts. Manage networks.' },
        { bold: 'Automations:', text: ' Automate remediation and maintenance. Pre-built tasks for disk cleanup, configuration, and more. Custom scripts.' },
        { bold: 'Expert services:', text: ' Get dedicated techs to augment project work, help desk services, and network ops. 24/7/365.' }
      ],
    ...
    var template_formUniqueSelector = "#mktoForm_1026";
...
function addSplitScreen(formSelector, template_content) {
  ...
  var formLoaded = setInterval(function () {
    if (formEl.querySelectorAll('input').length > 0) {
      ...
      var submitButtom = document.querySelector("#mktoForm_1026 .mktoButton");
      if (industryEmptyOption) industryEmptyOption.textContent = '';
      if (submitButtom) {
        submitButtom.classList.add('spz4006_v');
        submitButtom.textContent = 'Submit';
      }

    }
  }, 50);

  document.querySelector('.spz-split-wrap').insertAdjacentHTML('afterend', `<div class="spz-split-outer"></div>`);
  document.querySelector('.spz-split-outer').insertAdjacentElement('afterbegin', document.querySelector('.spz-split-wrap'));
}
...
function initExpandableHowField(formEl) {
  ...
  trigger.addEventListener('click', function () {
    ...
    if (expanded) {
      row.className += ' spz-expanded';
      field.focus();
    } else {
      row.className = row.className.replace(/ spz-expanded/g, '');
    }
...
// Body interval
var bodyInterval4006 = setInterval(function () {
...
(function () {
  ...
  const expName = '4006'; //experiment name should be 1001, 1002, 1003 etc.
